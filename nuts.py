...
def eigen_r0_sampler(eigenvals, eigenvecs, known_elementwise_prec=None):
    ...
    # first generate a random vector in the eigenvalue directions
    eigen_r0 = np.random.randn(eigenvals.shape[0])/eigen_sqrt

    # then a vector for all the other dimensions, from which we'll subtract the projections
    # on eigenvecs
    if known_elementwise_prec is None:
        r0 = np.random.randn(eigenvecs.shape[1])/eigen_sqrt.min()
    else:
        r0 = np.random.randn(eigenvecs.shape[1])/np.sqrt(known_elementwise_prec)

    r0 = r0 - (eigenvecs @ r0) @ eigenvecs
    r0 = r0 + eigen_r0 @ eigenvecs

    return r0

def nuts6(f, M, Madapt, theta0, delta=0.6, progress=False, r0_sampler=None):
    """
    ...
    r0_sampler: callable
        function which should generate a random array of the same size as theta0
        if None, we default to gaussian generation with the same variance in all directions
    ...
    for m in progress_range(1, M + Madapt, progress=progress):
        # Resample momenta.
        r0 = np.random.normal(0, 1, D) if r0_sampler is None else r0_sampler()
